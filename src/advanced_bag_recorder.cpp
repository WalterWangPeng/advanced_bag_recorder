...
AdvancedBagRecorder::AdvancedBagRecorder(ros::NodeHandle & nh) : nh_(nh), frame_idx_(0) {
    ...
    timer_ =
        nh_.createTimer(ros::Duration(split_time_), &AdvancedBagRecorder::splitBagCallback, this);
    ...
    tf_static_sub_ =
        nh.subscribe(tf_static_topic_, 1, &AdvancedBagRecorder::tfStaticCallback, this);
    global_costmap_sub_ =
        nh.subscribe(global_costmal_topic_, 1, &AdvancedBagRecorder::globalCostmapCallback, this);
    local_path_marker_sub_ = nh_.subscribe(
        local_path_marker_topic_, 1, &AdvancedBagRecorder::localPathMarkerCallback, this);

    local_path_sub_ =
        nh_.subscribe(local_path_topic_, 1, &AdvancedBagRecorder::coverageLocalPathCallback, this);

    // 订阅普通话题
    for (auto & topic : normal_topics_) {
        ros::Subscriber sub = nh_.subscribe<topic_tools::ShapeShifter>(
            topic, 10, boost::bind(&AdvancedBagRecorder::genericCallback, this, _1, topic));
        subs_.push_back(sub);
    }
...
void AdvancedBagRecorder::loadParams() {
    ...
    nh_.param(
        ...
        std::string("/decision_planning/mower_mission/obs_costmap"));
    ...
    nh_.param(
        "footprint_topic", footprint_topic_,
        std::string("/decision_planning/mower_mission/robot_footprint"));
    ...
    nh_.param(
        "global_costmal_topic", global_costmal_topic_,
        std::string("/move_base/global_costmap/costmap"));
    nh_.param("tf_static_topic", tf_static_topic_, std::string("/tf_static"));
    nh_.param(
        "local_path_topic", local_path_topic_,
        std::string("/decision_planning/mower_mission/coverage_local_path"));
    nh_.param(
        "local_path_marker_topic", local_path_marker_topic_,
        std::string("/decision_planning/mower_mission/local_path_marker"));

    XmlRpc::XmlRpcValue normal_topics_xml;
    if (nh_.getParam("normal_topics", normal_topics_xml) &&
        normal_topics_xml.getType() == XmlRpc::XmlRpcValue::TypeArray) {
    ...
    if (nh_.getParam("topic_max_msgs", topic_max_msgs_xml) &&
        topic_max_msgs_xml.getType() == XmlRpc::XmlRpcValue::TypeArray) {
...
void AdvancedBagRecorder::manageFolders() {
    if (!fs::exists(bag_root_path_)) fs::create_directories(bag_root_path_);
    ...
    std::sort(
        ...
        [](const std::pair<std::string, std::time_t> & a,
           const std::pair<std::string, std::time_t> & b) { return a.second < b.second; });
...
bool AdvancedBagRecorder::checkFreeSpaceOrExit() {
    ...
    if (free_gb < min_free_space_gb_) {
        ROS_ERROR(
            "Disk free space too low: %.2f GB < %.2f GB. Exiting.", free_gb, min_free_space_gb_);
        ros::shutdown();
        return false;
    }
    return true;
...
void AdvancedBagRecorder::splitBagCallback(const ros::TimerEvent &) {
    if (!checkFreeSpaceOrExit()) return;
...
void AdvancedBagRecorder::footprintCallback(const geometry_msgs::PolygonStamped::ConstPtr & msg) {
    ...
    if (current_bag_.isOpen()) {
        ...
        current_bag_.write(
            footprint_topic_, msg->header.stamp.isZero() ? ros::Time::now() : msg->header.stamp,
            *msg);
    }
}

void AdvancedBagRecorder::genericCallback(
    const topic_tools::ShapeShifter::ConstPtr & msg, const std::string & topic) {
    if (!checkFreeSpaceOrExit()) return;
    int max_msgs = -1;
    if (topic_max_msgs_map_.count(topic)) max_msgs = topic_max_msgs_map_[topic];
    if (max_msgs > 0 && topic_msg_count_[topic] >= max_msgs) return;
    if (ros::Time::now().isZero()) {
        ROS_WARN_THROTTLE(5, "ros::Time::now() is zero, waiting for /clock...");
        return;  // 等待时间有效
    }
...
cv::Point worldToImage(const geometry_msgs::Point & pt, const nav_msgs::MapMetaData & info) {
    int img_x = static_cast<int>((pt.x - info.origin.position.x) / info.resolution);
    int img_y =
        info.height - 1 - static_cast<int>((pt.y - info.origin.position.y) / info.resolution);
...
void AdvancedBagRecorder::openNewBag() {
    ...
    if (latest_coverage_local_path_) {
        current_bag_.write(local_path_topic_, ros::Time::now(), latest_coverage_local_path_);
    }
    if (latest_local_path_marker_) {
        current_bag_.write(local_path_marker_topic_, ros::Time::now(), latest_local_path_marker_);
    }
}
...
bool sameMarker(
    const visualization_msgs::Marker & marker_1, const visualization_msgs::Marker & marker_2) {
    if (marker_1.pose.position.x == marker_2.pose.position.x &&
        marker_1.pose.position.y == marker_2.pose.position.y) {
        return true;
    }
    return false;
}
...
bool samePose(
    ...
    if (pose_1.pose.position.x == pose_2.pose.position.x &&
...
void AdvancedBagRecorder::localPathMarkerCallback(
    const visualization_msgs::MarkerArray::ConstPtr & msg) {
    if (!msg || msg->markers.empty()) {
        return;
    }
    if (msg->markers.front().action == visualization_msgs::Marker::ADD &&
        record_local_path_marker_shoot_) {
        // 塞一个清除进去
        ...
        current_bag_.write(local_path_marker_topic_, ros::Time::now(), delete_marker_array);
        current_bag_.write(local_path_marker_topic_, ros::Time::now(), *msg);
        record_local_path_marker_shoot_ = false;
    }
}

void AdvancedBagRecorder::coverageLocalPathCallback(const nav_msgs::Path::ConstPtr & msg) {
    if (!latest_coverage_local_path_) {
        latest_coverage_local_path_ = msg;
        return;
    }
    if (latest_coverage_local_path_->poses.empty() ||
        latest_coverage_local_path_->poses.size() != msg->poses.size() ||
        !samePose(latest_coverage_local_path_->poses.front(), msg->poses.front()) ||
        !samePose(latest_coverage_local_path_->poses.back(), msg->poses.back())) {
        current_bag_.write(local_path_topic_, ros::Time::now(), *msg);
        record_local_path_marker_shoot_ = true;
    }
